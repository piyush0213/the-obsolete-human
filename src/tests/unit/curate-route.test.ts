/**
 * @file curate-route.test.ts
 * @description Implements tests/unit/curate-route.test.ts for The Obsolete Human Museum.
 */
import {
  describe,
  it,
...
describe('POST /api/curate', () => {
  ...
  describe('successful generation', () => {
    it('returns generated text for valid curator request', async () => {
      const res = await POST(
        makeRequest(
          {
            promptType: 'curator',
            userMessage: 'A person who still uses a wristwatch',
          },
          '10.0.0.1'
        )
      );
    ...
    it('returns generated text for valid habitat request', async () => {
      mockFetch.mockResolvedValueOnce(
        makeGeminiResponse(
          'The nesting chamber reveals ritualistic display behavior.'
        )
      );
      ...
      const res = await POST(
        makeRequest(
          {
            promptType: 'habitat',
            userMessage: 'A desk covered in cables and coffee cups',
          },
          '10.0.0.2'
        )
      );
    ...
    it('returns generated text for valid field_note request', async () => {
      mockFetch.mockResolvedValueOnce(
        makeGeminiResponse(
          'Subject 47 was observed pressing a button repeatedly.'
        )
      );
        ...
        makeRequest(
          {
            ...
            userMessage: 'Refreshing a webpage 30 times',
          },
          '10.0.0.3'
        )
      );
    ...
    it('calls Gemini API with correct system instruction and user message', async () => {
      await POST(
        makeRequest(
          {
            promptType: 'curator',
            userMessage: 'A person who irons their jeans',
          },
          '10.0.0.4'
        )
      );
  ...
  describe('validation failures', () => {
    it('returns 400 for missing promptType', async () => {
      const res = await POST(
        makeRequest({ userMessage: 'Some text' }, '10.0.1.1')
      );
    ...
    it('returns 400 for empty userMessage', async () => {
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: '' }, '10.0.1.3')
      );
    ...
    it('returns 400 for missing userMessage', async () => {
      const res = await POST(
        makeRequest({ promptType: 'curator' }, '10.0.1.5')
      );
  ...
  describe('rate limiting', () => {
    ...
    it('includes CORS headers on 429 responses', async () => {
      ...
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: 'blocked' }, testIp)
      );
  ...
  describe('Gemini API error handling', () => {
    it('returns 500 when Gemini returns a non-OK status', async () => {
      ...
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: 'Test' }, '10.0.3.1')
      );
    ...
    it('returns 500 when Gemini response contains an error field', async () => {
      ...
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: 'Test' }, '10.0.3.2')
      );
    ...
    it('returns 500 when Gemini returns empty candidates', async () => {
      ...
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: 'Test' }, '10.0.3.3')
      );
    ...
    it('never exposes the API key in error responses', async () => {
      ...
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: 'Test' }, '10.0.3.4')
      );
    ...
    it('returns a user-friendly message on internal errors', async () => {
      ...
      const res = await POST(
        makeRequest({ promptType: 'curator', userMessage: 'Test' }, '10.0.3.5')
      );
