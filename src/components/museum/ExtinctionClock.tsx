'use client';
/**
 * @file ExtinctionClock.tsx
 * @description Implements components/museum/ExtinctionClock.tsx for The Obsolete Human Museum.
 */
 ...
 * @returns {JSX.Element}
 */
function ExtinctionClockComponent({
  expiryDate,
  className,
}: ExtinctionClockProps): JSX.Element {
  const prefersReducedMotion = useReducedMotion();
  const currentYear = new Date().getFullYear();
  const clockItems = useMemo(() => {
    return EXTINCTION_CLOCK_DATA.map((item) => {
      const progress = calculateExtinctionProgress(item);
      const yearsText = formatYearsRemaining(
        item.estimatedExtinctionYear,
        currentYear
      );
      return { item, progress, yearsText };
    });
  }, [currentYear]);
    ...
    (remaining: number) => {
      ...
      for (const m of milestones) {
        if (days === m && lastMilestone[0] !== `${m}`) {
          lastMilestone[1](`${m}`);
          announce(
            `Warning: Only ${days} day${days !== 1 ? 's' : ''} remaining in your carbon budget.`
          );
          break;
        }
      }
  ...
  const totalBudgetMs = expiryDate
    ? Math.max(
        expiryDate.getTime() -
          (expiryDate.getTime() - 365.25 * 24 * 60 * 60 * 1000),
        1
      )
    : 1;
  const progressPercent = expiryDate
    ? Math.min(((totalBudgetMs - remaining) / totalBudgetMs) * 100, 100)
    : 0;
  ...
  return (
    <section
      aria-labelledby="extinction-clock-heading"
      className={cn('space-y-8', className)}
    >
      <header className="text-center space-y-2">
        <h2
          id="extinction-clock-heading"
          className="font-serif text-3xl text-museum-text tracking-wide"
        >
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative w-48 h-48">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-mono text-2xl text-museum-text">
                {days}
              </span>
              <span className="text-[10px] text-museum-text-muted uppercase tracking-widest">
                days left
              </span>
            </div>
          </div>
          ...
          {/* Digital countdown */}
          <div
            className="flex gap-4 font-mono text-sm text-museum-text-muted"
            aria-live="off"
          >
            {[
              { val: days, unit: 'Days' },
              ...
              <div key={unit} className="text-center">
                <span className="block text-lg text-museum-text tabular-nums">
      ...
      <div
        className="space-y-4"
        role="list"
        aria-label="Behavioral extinction timelines"
      >
        {clockItems.map(({ item, progress, yearsText }) => {
          return (
            <article
              key={item.behavior}
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="font-serif text-sm text-museum-text">
                  {item.behavior}
                </h3>
              ...
              <div
                className="relative w-full h-2 rounded-full bg-museum-bg-elevated overflow-hidden"
                role="progressbar"
              ...
              <div className="flex items-center justify-between text-[10px] text-museum-text-muted font-mono">
                <span>
                  Peak: {item.peakAdoptionRate}% ({item.peakAdoptionYear})
                </span>
...
export const ExtinctionClock = React.memo(ExtinctionClockComponent);
