...
const TRANSPORT_FACTORS: Record<HabitsInput["transport"], number> = {
  none: 0,
  public: 0.089,       // bus/metro average
  private: 0.21,       // petrol car average
  mixed: 0.14,
  aviation_heavy: 0.255, // includes frequent flying
};
...
const ELECTRONICS_FACTORS: Record<HabitsInput["electronicsReplacement"], number> = {
  rarely: 50,
  bi_yearly: 180,
  yearly: 350,
  obsessively: 700,
...
export function calculateAnnualEmissions(habits: HabitsInput): EmissionsKg {
  ...
  const deliveryKg =
    habits.deliveriesPerWeek * 52 * KG_PER_DELIVERY;
  const electronicsKg =
    ELECTRONICS_FACTORS[habits.electronicsReplacement];

  const total = transportKg + dietKg + energyKg + acKg + deliveryKg + electronicsKg;
...
export const SAMPLE_SPECIMENS: readonly Specimen[] = [
  ...
  },
  {
    ...
    habitat: "Corporate office towers, business parks, converted industrial spaces",
    carbonFootprint: {
      annualKg: 3200,
      equivalentTrees: 146,
      category: "HOUSING",
 ...
 */
export function calculateExtinctionProgress(
  data: ExtinctionClockData,
): number {
