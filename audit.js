...
function warn(msg) {
  console.log(`${YELLOW}${BOLD}⚠️ WARN:${RESET} ${msg}`);
}
...
if (fs.existsSync('src/lib/carbon-calculator.ts')) {
  const extinctionLogic = fs.readFileSync('src/lib/carbon-calculator.ts', 'utf8');
  if (extinctionLogic.includes('calculateAnnualEmissions')) {
    pass('Carbon calculation logic found in carbon-calculator.ts');
  } else {
    fail('No calculateAnnualEmissions logic found in carbon-calculator.ts', 10);
  }
}
...
if (anyCount > 0)
  fail(
    `Found ${anyCount} instances of 'any' types in src/`,
    Math.min(10, anyCount * 2)
  );
else pass("No 'any' types found in src/ files.");

if (consoleCount > 0)
  fail(
    `Found ${consoleCount} console.log statements in production code`,
    Math.min(5, consoleCount)
  );
...
try {
  ...
  fail(
    'TypeScript compilation failed (run `npx tsc --noEmit` manually to see errors)',
    10
  );
}
...
try {
  ...
  fail(
    'ESLint violations found (run `npx eslint .` manually to see errors)',
    5
  );
}
...
if (secretCount > 0)
  fail(`Found ${secretCount} potential hardcoded secrets!`, 20);
else pass('No hardcoded secrets found in source files.');

if (evalCount > 0)
  fail(`Found eval() or Function() usage. Highly insecure!`, 15);
else pass('No eval() or Function() used.');

if (innerHtmlCount > 0)
  fail(
    `dangerouslySetInnerHTML used! Ensure it is sanitized with DOMPurify.`,
    5
  );
...
if (fs.existsSync(apiRoutePath)) {
  ...
  if (
    apiRoute.includes("export const runtime = 'edge'") ||
    apiRoute.includes('export const runtime = "edge"')
  ) {
...
console.log(
  `${BOLD}FINAL PROJECT HEALTH SCORE: ${scoreColor}${score} / 100${RESET}`
);
...
if (recommendations.length > 0) {
  ...
  recommendations
    .slice(0, 5)
    .forEach((r, i) => console.log(`${YELLOW} ${i + 1}. ${r}${RESET}`));
}
